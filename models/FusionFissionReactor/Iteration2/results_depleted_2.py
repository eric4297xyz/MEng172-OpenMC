...
RESULTS_FILE = "results/iteration2_results_depleted_fuel/depletion_results.h5"
...
def extract_nuclide_inventories(data):
    ...
    key_nuclides = ['U235', 'U238', 'Pu239', 'Pu240', 'Pu241', 'Cm244', 'Xe135', 'Sm151', 'Cs137']
    results = {}
...
def generate_summary_report(data, metrics, inventories):
    ...
    # First, report key actinides explicitly
    key_actinides = ['U235', 'U238', 'Pu238', 'Pu239', 'Pu240', 'Pu241', 'Pu242', 
                     'Am241', 'Am243', 'Cm244', 'Np237', 'Sm151', 'Cs137']
    report.append("\nKey Actinides:")
    for nuc in key_actinides:
        if nuc in inventories:
            inv = inventories[nuc]
            initial = inv[0]
            final = inv[-1]
            if initial > 1e-10:
                rel_change = 100.0 * (final - initial) / initial
                report.append(f"  {nuc:>12s}: {initial:.4e} → {final:.4e} ({rel_change:+6.1f}%)")
            else:
                report.append(f"  {nuc:>12s}: {initial:.4e} → {final:.4e} (produced)")
