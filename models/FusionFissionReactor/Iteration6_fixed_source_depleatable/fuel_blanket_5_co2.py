...
def build_trufuelsphere_nobox(sphere_inner_radius, sphere_outer_radius):
    ...
    surrounded by carbon dioxide (CO2) coolant/moderator.
    NO aluminum box in this iteration - simplified geometry.

    This is Iteration4 modified for CO2 coolant study:
    ...
    - CO2 coolant/moderator instead of D2O (pressurized CO2 at ~0.8 g/cm³)
    ...
    # Carbon dioxide coolant/moderator (CO2)
    # Using pressurized CO2 at high density (typical for reactor coolant systems)
    co2_coolant = openmc.Material(name='CO2_coolant')
    co2_coolant.add_element('C', 1.0)   # Carbon
    co2_coolant.add_element('O', 2.0)   # Oxygen (2 atoms per CO2 molecule)
    co2_coolant.set_density('g/cm3', 0.8)  # Pressurized CO2 (~100 bar, ~50°C)
    co2_coolant.temperature = 300  # Room temperature (K)
    # Note: OpenMC doesn't have S(α,β) thermal scattering data for CO2

    materials = openmc.Materials([mtru_fuel, co2_coolant])
    ...
    inner_coolant_cell = openmc.Cell(
        name='inner_coolant_co2',
        region=-inner_surface_sphere
    )
    inner_coolant_cell.fill = co2_coolant
