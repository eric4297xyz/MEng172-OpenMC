...
def build_trufuelsphere_nobox(sphere_inner_radius, sphere_outer_radius):
    ...
    surrounded by graphite moderator.
    NO aluminum box in this iteration - simplified geometry.

    This is Iteration4 modified for graphite moderator study:
    - PURIFIED TRANSURANIC FUEL (no uranium) WITHOUT ALUMINUM BOX
    ...
    - Graphite moderator instead of D2O or CO2
    - Nuclear-grade graphite with thermal scattering
    - Simplified geometry: no aluminum box, just fuel shell with moderator
    ...
    # Nuclear-grade graphite moderator
    # Using standard nuclear graphite density (1.7 g/cm³)
    graphite = openmc.Material(name='graphite_moderator')
    graphite.add_element('C', 1.0)  # Natural carbon
    graphite.set_density('g/cm3', 1.7)  # Nuclear-grade graphite density
    graphite.temperature = 300  # Room temperature (K)
    # Add thermal scattering for graphite (critical for proper moderation)
    graphite.add_s_alpha_beta('c_Graphite')

    materials = openmc.Materials([mtru_fuel, graphite])

    # 2) Geometry - Simplified: just inner moderator region and fuel shell
    ...
    # Cells
    inner_moderator_cell = openmc.Cell(
        name='inner_graphite',
        region=-inner_surface_sphere
    )
    inner_moderator_cell.fill = graphite
