"""
FILE: spatial_mesh_plots_graphite.py
Create 2D spatial plots from mesh tallies in graphite moderator depletion results
...
def plot_2d_distribution(data, quantity_name, step_num, vmin=None, vmax=None):
    ...
    ax.set_title(f'{quantity_name.capitalize()} Distribution (CO₂) at z={z_value:.1f} cm\\n'
...
def plot_radial_profiles(data, step_num):
    ...
    fig.suptitle(f'Radial Profiles (CO₂ Coolant) at Step {step_num} (t={step_num*90:.0f} days)', 
...
def plot_3_panel_comparison(data, step_num, vmins=None, vmaxs=None):
    ...
    fig.suptitle(f'Spatial Distributions (CO₂ Coolant) at Step {step_num} (t={step_num*90:.0f} days, z=0 plane)',
                 fontsize=14, fontweight='bold')
...
def main():
    ...
    """
    print("="*70)
    print("SPATIAL MESH PLOTS FOR FUSION-FISSION HYBRID REACTOR - GRAPHITE MODERATOR")
    print("="*70)
    print(f"\\nResults: {RESULTS_DIR}")
    ...
    print("\\n" + "="*70)
    print("SPATIAL PLOTTING COMPLETE - GRAPHITE MODERATOR")
    print("="*70)
