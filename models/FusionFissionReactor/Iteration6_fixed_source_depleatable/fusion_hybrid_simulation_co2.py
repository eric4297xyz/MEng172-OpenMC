#!/usr/bin/env python3
"""
Fusion-Fission Hybrid Reactor Simulation with Fixed-Source Depletion - CO2 COOLANT STUDY
...
MODIFICATION: Uses CO2 as coolant/moderator instead of D2O (heavy water)

System Description:
- External D-D fusion neutron source (2.45 MeV, 10^14 n/s)
- Subcritical spent fuel blanket (k_source ~ 1.1)
- Spherical geometry with CO2 coolant/moderator (pressurized)
...
- CO2 coolant/moderator study

Author: CO2 Coolant Study
Date: February 2026
"""
...
from fuel_blanket_5_co2 import build_trufuelsphere_nobox
...
# Output directory (NEW: CO2 study subdirectory)
output_dir = os.path.join(SCRIPT_DIR, "results_manual_depletion", "co2_coolant_study")
os.makedirs(output_dir, exist_ok=True)

print("="*70)
print("D-D FUSION-FISSION HYBRID REACTOR SIMULATION - CO2 COOLANT STUDY")
print("="*70)
...
print(f"  - CO2 coolant/moderator (pressurized, 0.8 g/cm³)")
...
print("\nBuilding geometry with CO2 coolant...")
(geometry, materials) = build_trufuelsphere_nobox(
    sphere_inner_radius=50.0,
    sphere_outer_radius=100.0
)
print("Geometry: Fuel shell (r=50-100 cm) with inner CO2 coolant")
...
print("\n" + "="*70)
print("STARTING FIXED-SOURCE DEPLETION SIMULATION - CO2 COOLANT")
print("="*70)
...
print("\n" + "="*70)
print("SIMULATION COMPLETE - CO2 COOLANT STUDY")
print("="*70)
...
with open(os.path.join(output_dir, 'depletion_summary_co2.txt'), 'w') as f:
    f.write("D-D FUSION-FISSION HYBRID REACTOR - DEPLETION RESULTS (CO2 COOLANT)\n")
    f.write("="*70 + "\n\n")
    f.write(f"Coolant: Carbon Dioxide (CO2) at 0.8 g/cm³ (pressurized)\n")
...
print(f"\nSummary saved to: {os.path.join(output_dir, 'depletion_summary_co2.txt')}")
print("\nNOTE: This simulation uses CO2 coolant/moderator instead of D2O")
print("Compare results with the D2O baseline in ../results_manual_depletion/")
print("="*70)
