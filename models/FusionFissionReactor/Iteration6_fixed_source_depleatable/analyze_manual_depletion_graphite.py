#!/usr/bin/env python3
"""
Comprehensive Analysis of Manual Depletion Results - GRAPHITE MODERATOR STUDY
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_manual_depletion", "graphite_moderator_study")
...
def parse_summary_file():
    ...
    summary_file = os.path.join(RESULTS_DIR, 'depletion_summary_co2.txt')
...
def create_detailed_summary():
    ...
    print("="*70)
    print("ANALYZING MANUAL DEPLETION RESULTS - CO2 COOLANT")
    print("="*70)
    ...
    with open(OUTPUT_FILE, 'w') as f:
        f.write("="*70 + "\n")
        f.write("ITERATION 6: GRAPHITE MODERATOR STUDY RESULTS\n")
        f.write("Fusion-Fission Hybrid Reactor with Nuclear-Grade Graphite Moderator\n")
        f.write("="*70 + "\n\n")
        
        f.write("SOURCE CONFIGURATION:\n")
        f.write("-"*70 + "\n")
        f.write(f"  Fusion Type:         D-D fusion\n")
        f.write(f"  Coolant:             Carbon Dioxide (CO₂) at 0.8 g/cm³\n")
        ...
        f.write("="*70 + "\n")
        f.write("PHYSICS INTERPRETATION - CO2 COOLANT\n")
        f.write("="*70 + "\n")
        f.write(f"This is a D-D fusion driven subcritical reactor system with CO₂ coolant:\n\n")
        ...
        f.write(f"3. CO₂ coolant characteristics:\n")
        f.write(f"   - Density: 0.8 g/cm³ (pressurized)\n")
        f.write(f"   - Carbon provides some moderation\n")
        f.write(f"   - Less effective moderator than D₂O (heavy water)\n")
        f.write(f"   - Expected higher k_source compared to light water\n\n")
        
        f.write(f"4. System behavior:\n")
        ...
        f.write(f"5. Energy multiplication:\n")
        if step_data:
        ...
        f.write("="*70 + "\n")
        f.write("COMPARISON NOTE\n")
        f.write("="*70 + "\n")
        f.write("Compare these results with the D₂O baseline in:\n")
        f.write("  ../results_manual_depletion/analysis_summary.txt\n")
        f.write("\nKey differences expected:\n")
        f.write("  - Different neutron spectrum (less thermalization with CO₂)\n")
        f.write("  - Different k_source due to moderation differences\n")
        f.write("  - Different spatial distributions\n")
        f.write("="*70 + "\n")
...
def main():
    """Main execution."""
    print("="*70)
    print("MANUAL DEPLETION ANALYSIS - GRAPHITE MODERATOR STUDY")
    print("="*70)
    print()
