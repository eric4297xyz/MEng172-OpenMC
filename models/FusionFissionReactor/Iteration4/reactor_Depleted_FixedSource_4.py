# FILE: reactor_Depleted_FixedSource_4.py
# Iteration4: Fixed source depletion simulation with TRU-only fuel (no uranium)
# Changes from Iteration3:
#   - Point source at origin instead of cylindrical source
#   - Removed aluminum box (simplified geometry)
...
# Depletion time: 182.5 days (reduced to 1 step for faster completion)
days_per_year = 365.0
hours_per_day = 24.0
seconds_per_hour = 3600.0
time_years = 0.5
time_seconds = time_years * days_per_year * hours_per_day * seconds_per_hour
num_steps = 1  # Reduced from 3 to 1 to avoid hanging
...
print("="*70)
print("ITERATION 4: TRU FUEL (NO URANIUM) - SIMPLIFIED GEOMETRY")
print("="*70)
...
print("  - POINT SOURCE at origin (0,0,0)")
print("  - NO aluminum box (simplified geometry)")
print("  - Inner coolant region with fuel shell")
print("")

print("Building model...")
# Create point source at origin
my_source = openmc.IndependentSource()
my_source.space = openmc.stats.Point((0, 0, 0))  # Point source at origin
my_source.angle = openmc.stats.Isotropic()  # Isotropic emission
my_source.energy = openmc.stats.Uniform(E_min, E_max)  # Energy distribution
my_source.strength = source_rate
print(f"Point source at origin (0,0,0)")
...
print("  - Use results_depleted_fixedsource_4.py to analyze")
print(f"{'='*70}")
